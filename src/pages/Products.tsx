        ...
        <div className="container py-10 sm:py-14">
          <nav aria-label="breadcrumb" className="text-sm text-muted-foreground">
            <ol className="flex items-center gap-2">
              ...
              <li aria-hidden>›</li>
              <li className="text-foreground">{t.products.breadcrumbProducts}</li>
            </ol>
          </nav>
        ...
        <div className="container">
          {!isLoading && total > 0 && (
            <p className="mb-6 text-sm text-muted-foreground">
              {t.products.showing}{" "}
              <span className="font-semibold text-foreground">{toBnNumber(total)}</span>
              {t.products.productsUnit}
            </p>
          )}
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-border bg-muted/30 px-6 py-20 text-center">
              ...
              <p className="mt-1 text-sm text-muted-foreground">{t.products.emptyDesc}</p>
            </div>
          ) : (
            <div
              className={`grid grid-cols-2 gap-5 transition-opacity sm:grid-cols-3 lg:grid-cols-6 ${
              ...
              {pageNumbers.map((p, i) =>
                p === "…" ? (
                  ...
                  <Button
                    ...
                    aria-label={`${t.home.next} ${toBnNumber(p)}`}
                  >
                    <span className="text-sm font-medium">{toBnNumber(p)}</span>
                  </Button>
                ),
              )}
