        ...
        {options.map((o, i) => (
          <div key={o.id} className={`flex items-center gap-1.5 rounded-md border px-2 py-1 ${correct === o.id ? "border-success bg-success/10" : "border-border"}`}>
            ...
            <Input value={o.text} onChange={(e) => setOpt(i, e.target.value)}
              className="h-7 border-0 bg-transparent px-1 text-xs focus-visible:ring-0" placeholder={`অপশন ${i + 1}`} />
          </div>
        ))}
      </div>
      {dirty && <Button size="sm" className="ml-7" onClick={save}><Save className="mr-1 h-3 w-3" /> সংরক্ষণ</Button>}
    </div>
  );
}
